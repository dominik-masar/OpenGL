...
// The texture with object normals.
...
void main()
{
	...
	vec3 normal_from_texture = textureProj(normal_tex, snow_tex_coord).rgb * 2.0 - 1.0;

	// Apply normal mapping.
	float e = 1.0 / textureSize(accumulated_snow_tex, 0).x;
	float scaleX = 26.0;
	float scaleY = 0.73;
	float scaleZ = 26.0;
	float r = textureProj(accumulated_snow_tex, snow_tex_coord + vec4( e,  0, 0, 0)).r;
	float l = textureProj(accumulated_snow_tex, snow_tex_coord + vec4(-e,  0, 0, 0)).r;
	float t = textureProj(accumulated_snow_tex, snow_tex_coord + vec4( 0,  e, 0, 0)).r;
	float b = textureProj(accumulated_snow_tex, snow_tex_coord + vec4( 0, -e, 0, 0)).r;

	vec3 T = normalize(vec3(2*e*scaleX, (t-b)*scaleY, 0));
	vec3 B = normalize(vec3(0, (r-l)*scaleY, 2*e*scaleZ));
	vec3 N = normalize(cross(B,T));

	N = normalize(T * normal_from_texture.x + B * normal_from_texture.y + N *normal_from_texture.z);
