...
// Snow
// ----------------------------------------------------------------------------
// The texture with the depth rendered from the light point of view.
layout (binding = 1) uniform sampler2D ortho_tex;
layout (binding = 2) uniform sampler2D accumulated_snow_tex;
// The premultiplied shadow matrix.
uniform mat4 snow_matrix;
uniform bool use_snow = false;
...
void main()
{
	...
	if (use_snow) {
		vec4 snow_tex_coord = snow_matrix * vec4(in_data.position_ws, 1.0);
		float dist_in_tex = textureProj(ortho_tex, snow_tex_coord).x;
	
		if (!(dist_in_tex + 0.000005 < snow_tex_coord.z / snow_tex_coord.w)) {
			snow_factor = dot(in_data.normal_ws, vec3(0.0, 1.0, 0.0)); // computes snow factor
	...
	vec3 mat_ambient = has_texture ? texture(material_diffuse_texture, in_data.tex_coord*uv_multiplier).rgb :  material.ambient;
	vec3 mat_diffuse = has_texture ? texture(material_diffuse_texture, in_data.tex_coord*uv_multiplier).rgb :  material.diffuse;
	vec3 mat_specular = material.specular;

	// Apply snow
	mat_diffuse = mix(mat_diffuse, vec3(1.0), snow_factor);
